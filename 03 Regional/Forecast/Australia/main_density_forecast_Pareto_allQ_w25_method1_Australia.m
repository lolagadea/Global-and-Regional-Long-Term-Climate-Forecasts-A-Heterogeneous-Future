% ========================================================================
% main_density_forecast_Pareto_allQ_w25_method1_Australia.m
% ------------------------------------------------------------------------
% Density forecasts using Pareto-superior models, all selected quantiles.
% Region: Australia. Window: w = 25. Method 1.
% ========================================================================
...
RDOS_FILE = fullfile(RES_DIR,'RDOS_Australia_1960_2023.mat');
BIC_FILE  = fullfile(RES_DIR,'BIC_Australia.dat');
...
assert(isfile(RDOS_FILE),   'RDOS_Australia_1960_2023.mat not found.');
assert(isfile(BIC_FILE),    'BIC_Australia.dat not found.');
...
QUANTILES = QUANTILES_monthly.Australia;
...
% ------------------------------------------------------------------------
% 4. Density forecast: Pareto-superior models, method 1
% ------------------------------------------------------------------------
...
for j = 1:length(W)
    ...
    for i = 1:length(F)
        ...
        for q = 1:length(Q)
            ...
            fprintf('Method 1: Australia, h=%d, quantile index=%d\n',f,Q(q));
            ...
            % Legacy rule:
            % For h=10, use second-best loss models because first-best
            % produced very unusual results in the original code.
            if i == 2
                selected_models_loss = selected_models_loss2;
            end
            ...
            BIC_q = BIC(Q(q),selected_models_loss==1);

            Weig_sbic = NaN(1,m);

            for k = 1:m
                Weig_sbic(k) = exp(-1/2*BIC_q(k)) / ...
                    sum(exp(-1/2*BIC_q(1,:)));
            end
            ...
            if length(find(selected_models_loss==1)) == 1

                varres = RES'*RES/t;
                CI_for_comb = FOR_comb + norminv([0.05 0.95])*sqrt(varres);

            elseif length(find(selected_models_loss==1)) > 1

                [ci_for_sbic, ci_models] = ...
                    compute_CI_comb_select_models( ...
                    QUANTILES,RDOS,Q(q),w,f,selected_models_loss,Weig_sbic,1); %#ok<ASGLU>
...
tableCaption = ['Long-term density forecast with Pareto superior model ' ...
    'for all quantiles (Australia, CRU data, 1960--2023, w=25, method1)'];

tableLabel = 'tab-Pareto-one-model-all-quantiles-method1-Australia-1960-2023';

file = fullfile(TAB_DIR, ...
    'Table_onemodel_allquantiles_w25_method1_for_Australia_1960_2023.tex');
...
file = fullfile(TAB_DIR, ...
    'Table_onemodel_allquantiles_w25_method1_fit_Australia_1960_2023.tex');
...
for i = 1:length(F)
    ...
    fprintf('\nThe selected models with method 1, h=%d, are:\n',F(i));
...
save(fullfile(RES_DIR,'RDOS_FOR_allQ_method1_w25_Australia_1960_2023.mat'), ...
    'RDOS_FOR');

save(fullfile(RES_DIR,'RDOS_FIT_allQ_method1_w25_Australia_1960_2023.mat'), ...
    'RDOS_FIT');

fprintf('\nDensity forecast method1 completed for Australia.\n');
