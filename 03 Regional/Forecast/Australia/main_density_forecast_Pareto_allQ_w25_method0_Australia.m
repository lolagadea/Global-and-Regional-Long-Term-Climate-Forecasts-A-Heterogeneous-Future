% ========================================================================
% main_density_forecast_Pareto_allQ_w25_method0_Australia.m
% ------------------------------------------------------------------------
% Density forecasts using Pareto-superior models, all selected quantiles.
% Region: Australia. Window: w = 25. Method 0.
% ========================================================================
...
% ------------------------------------------------------------------------
% 4. Density forecast: Pareto-superior models, method 0
% ------------------------------------------------------------------------
...
for j = 1:length(W)
    ...
    for i = 1:length(F)
        ...
        for q = 1:length(Q)
            ...
            selected_models = RDOS_GW.GW.models(Q(q),1:mod);
            ...
            BIC_q = BIC(Q(q),selected_models==1);
            ...
            if length(find(selected_models==1)) == 0

                disp('Warning: there are no selected models.')

            elseif length(find(selected_models==1)) == 1
...
tableCaption = ['Long-term density forecast with Pareto superior model ' ...
    'for all quantiles (Australia, CRU data, 1960--2023, w=25, method0)'];

tableLabel = 'tab-Pareto-one-model-all-quantiles-method0-Australia-1960-2023';

file = fullfile(TAB_DIR, ...
    'Table_onemodel_allquantiles_w25_method0_for_Australia_1960_2023.tex');
...
file = fullfile(TAB_DIR, ...
    'Table_onemodel_allquantiles_w25_method0_fit_Australia_1960_2023.tex');
...
save(fullfile(RES_DIR,'RDOS_FOR_allQ_method0_w25_Australia_1960_2023.mat'), ...
    'RDOS_FOR');

save(fullfile(RES_DIR,'RDOS_FIT_allQ_method0_w25_Australia_1960_2023.mat'), ...
    'RDOS_FIT');

fprintf('\nDensity forecast method0 completed for Australia.\n');
