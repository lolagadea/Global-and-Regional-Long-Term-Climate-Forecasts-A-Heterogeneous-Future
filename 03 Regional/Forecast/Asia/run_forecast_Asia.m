...
% run_forecast_Asia.m
% ------------------------------------------------------------------------
% Forecast competition for Asia temperature distribution characteristics.
...
THIS_DIR     = fileparts(mfilename('fullpath'));   % .../03_Regional/Forecast/Asia
...
for j = 1:length(w)
    ...
    for i = 1:length(h)
        ...
        fprintf('\nRunning Asia: window = %d, horizon = %d\n', w(j), h(i));
...
OUT_FILE = fullfile(RES_DIR,'RDOS_Asia_1960_2023.mat');
save(OUT_FILE,'RDOS');
...
fprintf('\nForecast competition completed for Asia.\n');
