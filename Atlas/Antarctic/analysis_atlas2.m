...
% ------------------------------------------------------------------------
% Density forecasts vs Atlas projections: Antarctic.
% Portable version.
% ========================================================================
...
FORECAST_RES_DIR = fullfile(CODES_DIR, ...
    '03 Regional','Forecast','Antarctic','Results_for');

% ------------------------------------------------------------------------
% 2. Load forecast distributions
% ------------------------------------------------------------------------
load(fullfile(FORECAST_RES_DIR, ...
    'for_2100_Antarctic_1960_2023.mat'))
for_2100 = FOR_comb.m2.for;

load(fullfile(FORECAST_RES_DIR, ...
    'RDOS_FIT_allQ_method2_w25_Antarctic_1960_2023.mat'))
for_2050 = RDOS_FIT(:,1,1,3);

% ------------------------------------------------------------------------
% 3. Load Atlas temperature levels
% ------------------------------------------------------------------------
% RCP2.6 is not available for Antarctic in this exercise.
atlas_2048_r26 = NaN;
atlas_2100_r26 = NaN;

load TEMP_2006_2100_Antarctic_rcp45
atlas_2048_r45 = TEMP.data.yearly(43);
atlas_2100_r45 = TEMP.data.yearly(95);

load TEMP_2006_2100_Antarctic_rcp85
...
% 5. Export
% ------------------------------------------------------------------------
OUT_NAME = fullfile(FIG_DIR,'Figure_atlas_Antarctic');
