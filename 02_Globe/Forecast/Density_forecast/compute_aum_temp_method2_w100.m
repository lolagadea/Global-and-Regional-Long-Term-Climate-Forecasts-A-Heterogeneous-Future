...
% ------------------------------------------------------------------------
% Compute temperature increases implied by Method 2 density forecasts and
...
% Pareto-superiority criterion and combined using BIC weights.
%
...
assert(exist(FILE_RDOS_FOR,'file')==2, ...
    'Missing file: %s. Run main_density_forecast_Pareto_allQ_w100_method2.m first.', FILE_RDOS_FOR);
assert(exist(FILE_QUANT,'file')==2, ...
    'Missing file: %s. Run the Introduction quantile construction first.', FILE_QUANT);
assert(exist(FILE_RDOS_ONE,'file')==2, ...
    'Missing file: %s. Run the forecast competition first.', FILE_RDOS_ONE);
...
select_w = 3;                     % w = 100 in forecast-competition objects
select_h_long = 3;                % use h = 25 selected set for long-run extrapolation
...
% h = 1, 10, 25
for ih = 1:3
    aum_temp(:,:,ih)      = squeeze(RDOS_m2(:,1,ih)) - MATRIX_mean;
    aum_temp_ci05(:,:,ih) = squeeze(RDOS_m2(:,2,ih)) - MATRIX_mean;
    aum_temp_ci95(:,:,ih) = squeeze(RDOS_m2(:,3,ih)) - MATRIX_mean;
end
...
for iq = 1:length(Q)
    ...
    selected_models = RDOS_one_model.select(:,select_w,select_h_long); % Method 2
...
save(fullfile(RES_DIR,'AUM_temp_Globe_1880_2023_method2_w100.mat'),'AUM_temp');
...
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100'),'-dpng');
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100'),'-dpdf');
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100'),'-deps');
...
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100_bis'),'-dpng');
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100_bis'),'-dpdf');
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method2_w100_bis'),'-deps');
...
writetable(Mean_increase,fullfile(TAB_DIR,'Mean_temperature_increase_method2_w100.csv'));
