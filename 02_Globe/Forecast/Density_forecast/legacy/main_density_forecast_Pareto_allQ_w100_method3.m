% ========================================================================
% main_density_forecast_Pareto_allQ_w100_method3.m
% ------------------------------------------------------------------------
% Density forecasts for all selected quantiles using Pareto-superior models.
% Method 3: take the union of the Method-2 selected models across
% forecast horizons and combine that common union using BIC weights.
%
...
for j = 1:length(W)
    ...
    % --------------------------------------------------------------------
    % Method 3 selection rule
    % --------------------------------------------------------------------
    % First retrieve the Method-2 selections separately for each horizon.
    % Then take their union: a model is included if it is selected for at
    % least one forecast horizon. The same union set is used for all horizons.
    % This is different from Method 2, where the selected set may vary with h.
    % --------------------------------------------------------------------

    selected_models_by_h = NaN(length(F),mod);

    for ii = 1:length(F)
        selected_models_by_h(ii,:) = RDOS_one_model.select(:,select_w,ii)';
    end

    SELECTED_models = false(1,mod);
    for mm = 1:mod
        SELECTED_models(mm) = any(selected_models_by_h(:,mm)==1);
    end
    ...
    for i = 1:length(F)
        ...
        selected_models_inters = SELECTED_models; % union across horizons

        for q = 1:length(Q)

            fprintf('Method 3 | w=%d | h=%d | quantile index=%d\n', w, f, Q(q));
...
caption_for = ['Long-term density forecast with Pareto-superior models ' ...
    'for all quantiles (the Globe, CRU data, 1880--2023, w=100, method 3)'];
label_for = 'tab-Pareto-one-model-all-quantiles-method3-Globe-1880-2023';

write_density_table( ...
    fullfile(TAB_DIR,'Table_onemodel_allquantiles_w100_method3_for_Globe_1880_2023.tex'), ...
    RDOS_FOR, F, rows, caption_for, label_for);

caption_fit = ['Long-term density forecast with Pareto-superior models ' ...
    'for all quantiles, fit uncertainty (the Globe, CRU data, 1880--2023, w=100, method 3)'];
label_fit = 'tab-Pareto-one-model-all-quantiles-method3-fit-Globe-1880-2023';

write_density_table( ...
    fullfile(TAB_DIR,'Table_onemodel_allquantiles_w100_method3_fit_Globe_1880_2023.tex'), ...
...
fprintf('\nThe selected models with method 3 (union across h=1,10,25) are:\n');
disp(models(SELECTED_models==1)');

% ------------------------------------------------------------------------
% 7. Save results
% ------------------------------------------------------------------------

save(fullfile(RES_DIR,'RDOS_FOR_allQ_method3_w100_Globe_1880_2023.mat'), ...
    'RDOS_FOR');

save(fullfile(RES_DIR,'RDOS_FIT_allQ_method3_w100_Globe_1880_2023.mat'), ...
    'RDOS_FIT');
