...
% ------------------------------------------------------------------------
% Compute temperature increases implied by Method 1 density forecasts and
...
FILE_RDOS_FOR = fullfile(RES_DIR,'RDOS_FOR_allQ_method1_w100_Globe_1880_2023.mat');
FILE_QUANT    = fullfile(INTRO_RES_DIR,'QUANTILES_monthly_Globe_1880_2023.mat');
FILE_MODELS   = fullfile(COMP_RES_DIR,'selected_the_model_allQ.mat');
FILE_MODELS2  = fullfile(COMP_RES_DIR,'selected_the_model2_allQ.mat');
FILE_BIC      = fullfile(COMP_RES_DIR,'BIC.dat');

assert(exist(FILE_RDOS_FOR,'file')==2, ...
    'Missing file: %s. Run main_density_forecast_Pareto_allQ_w100_method1.m first.', FILE_RDOS_FOR);
assert(exist(FILE_QUANT,'file')==2, ...
    'Missing file: %s. Run the Introduction quantile construction first.', FILE_QUANT);
assert(exist(FILE_MODELS,'file')==2, ...
    'Missing file: %s. Run the forecast competition first.', FILE_MODELS);
assert(exist(FILE_MODELS2,'file')==2, ...
    'Missing file: %s. Run the forecast competition first.', FILE_MODELS2);
...
load(FILE_MODELS,'MODELS');
load(FILE_MODELS2,'MODELS2');
...
% h = 1, 10, 25
for ih = 1:3
    aum_temp(:,:,ih)      = squeeze(RDOS_m1(:,1,ih)) - MATRIX_mean;
    aum_temp_ci05(:,:,ih) = squeeze(RDOS_m1(:,2,ih)) - MATRIX_mean;
    aum_temp_ci95(:,:,ih) = squeeze(RDOS_m1(:,3,ih)) - MATRIX_mean;
end
...
for iq = 1:length(Q)
    ...
    selected_models = (MODELS(:,3,3))'; % Method 1

    if isempty(find(selected_models==1,1))
        disp('Warning: no selected models in MODELS; using MODELS2 instead.');
        selected_models = (MODELS2(:,3,3))'; % Method 1 fallback
    end
...
save(fullfile(RES_DIR,'AUM_temp_Globe_1880_2023_method1_w100.mat'),'AUM_temp');
...
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100'),'-dpng');
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100'),'-dpdf');
print(fig1,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100'),'-deps');
...
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100_bis'),'-dpng');
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100_bis'),'-dpdf');
print(fig2,fullfile(FIG_DIR,'Figure_Globe_1880_2023_f2100_method1_w100_bis'),'-deps');
...
writetable(Mean_increase,fullfile(TAB_DIR,'Mean_temperature_increase_method1_w100.csv'));
