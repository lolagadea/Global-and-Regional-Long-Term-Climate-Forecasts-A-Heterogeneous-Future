% ========================================================================
% main_comb_with_Pareto_mean_w100.m
% ------------------------------------------------------------------------
% Forecast combinations for the global mean using Pareto-superior models.
% Window: w = 100.
% ========================================================================
...
Q = 1;
names_q = {'mean'}; %#ok<NASGU>

mod = 14;
nmodels_table = 17;

RDOS_df.for = NaN(3,3,length(F),length(W),length(Q));
RDOS_df.fit = NaN(3,3,length(F),length(W),length(Q));
...
for v = 1:length(Q)
    ...
    figfile = fullfile(FIG_DIR, ...
        'Figure_for_mean_comb_Pareto_Globe_1880_2023_w100');
...
TAB_FILE_FOR = fullfile(TAB_DIR, ...
    'Table_forecast_Pareto_mean_ci_for_w100_Globe_1880_2023.tex');
...
TAB_FILE_FIT = fullfile(TAB_DIR, ...
    'Table_forecast_Pareto_mean_ci_fit_w100_Globe_1880_2023.tex');
...
RDOS_mean.comb_Pareto.H = F;
RDOS_mean.comb_Pareto.W = W;
RDOS_mean.comb_Pareto.models = models;
RDOS_mean.comb_Pareto.forecast = FORECAST_table;
RDOS_mean.comb_Pareto.ci.fit = CI_fit_table;
RDOS_mean.comb_Pareto.ci.for = CI_for_table;
RDOS_mean.comb_Pareto.selected = SELECTED;
RDOS_mean.comb_Pareto.RDOS_df = RDOS_df;

RDOS = RDOS_mean;

save(RDOS_FILE,'RDOS');

fprintf('\nPareto-combined forecasts for the global mean completed.\n');
